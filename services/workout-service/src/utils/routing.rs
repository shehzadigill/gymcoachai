...
impl RouteMatcher {
    ...
    pub fn match_route(method: &str, path: &str) -> Option<Route> {
        match (method, path) {
            ...
            ("GET", path) if path.starts_with("/api/workouts/plans/") && !path.contains("/schedule") => Some(Route::GetWorkoutPlan),
            ("PUT", "/api/workouts/plans") => Some(Route::UpdateWorkoutPlan),
            ("DELETE", path) if path.starts_with("/api/workouts/plans/") => Some(Route::DeleteWorkoutPlan),
            
            // Workout Session routes
            ("GET", "/api/workouts/sessions") => Some(Route::GetWorkoutSessions),
            ("POST", "/api/workouts/sessions") => Some(Route::CreateWorkoutSession),
            ("GET", path) if path.starts_with("/api/workouts/sessions/") => Some(Route::GetWorkoutSession),
            ("PUT", "/api/workouts/sessions") => Some(Route::UpdateWorkoutSession),
            ("DELETE", path) if path.starts_with("/api/workouts/sessions/") => Some(Route::DeleteWorkoutSession),
            
            // Exercise routes
            ("GET", "/api/workouts/exercises") => Some(Route::GetExercises),
            ("POST", "/api/workouts/exercises") => Some(Route::CreateExercise),
            ("GET", path) if path.starts_with("/api/workouts/exercises/") && !path.ends_with("/clone") => Some(Route::GetExercise),
            ("PUT", "/api/workouts/exercises") => Some(Route::UpdateExercise),
            ("POST", path) if path.starts_with("/api/workouts/exercises/") && path.ends_with("/clone") => Some(Route::CloneExercise),
            ("DELETE", path) if path.starts_with("/api/workouts/exercises/") => Some(Route::DeleteExercise),
            ...
            ("POST", path) if path.starts_with("/api/workouts/plans/") && path.contains("/schedule") => Some(Route::ScheduleWorkoutPlan),
            ("GET", "/api/workouts/schedules") => Some(Route::GetScheduledWorkouts),
            ("PUT", path) if path.starts_with("/api/workouts/schedules/") => Some(Route::UpdateScheduledWorkout),
            ("DELETE", path) if path.starts_with("/api/workouts/schedules/") => Some(Route::DeleteScheduledWorkout),
    ...
    fn is_workout_plan_path(path: &str) -> bool {
        path.starts_with("/api/workouts/plans") || 
        path.starts_with("/workouts/plans")
    }

    /// Check if path is a workout session path
    fn is_workout_session_path(path: &str) -> bool {
        path.starts_with("/api/workouts/sessions") || 
        path.starts_with("/workouts/sessions")
    }

    /// Check if path is an exercise path
    fn is_exercise_path(path: &str) -> bool {
        path.starts_with("/api/workouts/exercises") || 
        path.starts_with("/workouts/exercises")
    }
