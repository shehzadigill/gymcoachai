    ...
    pub fn match_route(method: &str, path: &str) -> Option<Route> {
        match (method, path) {
            ...
            ("POST", "/api/analytics/strength-progress") => {
                Some(Route::CreateStrengthProgress)
            }
            ...
            ("POST", "/api/analytics/body-measurements") => {
                Some(Route::CreateBodyMeasurement)
            }
            ...
            ("POST", "/api/analytics/charts") => {
                Some(Route::CreateProgressChart)
            }
            ...
            ("POST", "/api/analytics/reports") => {
                Some(Route::GenerateProgressReport)
            }
            ...
            ("PUT", path) if path.contains("/progress-photos") => {
                Some(Route::UpdateProgressPhoto)
            }
