...
impl NutritionRepository {
    ...
    pub async fn get_nutrition_stats(&self, user_id: &str) -> Result<Value> {
        // Get today's date
        let today = Utc::now();
        ...
        // Get today's meals
        let today_meals = self.get_meals_by_date(user_id, &today).await.unwrap_or_default();
        
        // Calculate today's totals
        let mut today_calories = 0.0;
        let mut today_protein = 0.0;
        let mut today_carbs = 0.0;
        let mut today_fat = 0.0;
        let mut today_fiber = 0.0;
        let mut today_sodium = 0.0;
        ...
        let mut weekly_calories = 0.0;
        let mut weekly_days = 0;
        let mut weekly_meals = 0;
        
        for i in 0..7 {
            let day = week_start + chrono::Duration::days(i);
            if let Ok(day_meals) = self.get_meals_by_date(user_id, &day).await {
        ...
        loop {
            if let Ok(day_meals) = self.get_meals_by_date(user_id, &current_date).await {
                if day_meals.is_empty() {
                    break;
                }
                streak += 1;
        ...
        let today_water = self.get_water_by_date(user_id, &today_str).await.unwrap_or(None);
